...
// A fixed entry date keeps regeneration byte-for-byte reproducible; otherwise
// every run dirties the checked-in fixtures with new zip timestamps.
const DOCX_ENTRY_OPTIONS = { date: new Date(Date.UTC(2026, 0, 1, 0, 0, 0)) };
...
function buildDocx(paragraphs: string[]): Uint8Array {
  ...
  zip.file('word/document.xml', buildDocumentXml(paragraphs), DOCX_ENTRY_OPTIONS);
